...
function Evaluation() {
  ...
  const handleButtonClick = (user) => {
    handleEvaluate(user);
    navigate(`/evaluate/${user.id}`, { state: { user } });
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-4">
      <h1 className="text-lg font-semibold mb-4 text-black">누구의 당도를 평가할까요?</h1>
      <p className="mb-8 text-gray-500 text-xs">당도 평가를 통해 사용자의 당도 게이지가 결정돼요</p>
      <div className="w-full border-b mb-4 border-gray2"></div>
      
      {filteredUsers.length === 0 ? (
        <div className='flex flex-col items-center justify-center'>
          <img src={Empty} alt="평가할 구름이 없음" className="w-32 mt-4 mb-6" />
          <p className="text-black font-semibold">평가할 구름이 없어요 :(</p>
          <p className="mb-4 text-gray-500 text-sm">멘토링으로 새로운 인연을 만나봐요!</p>
